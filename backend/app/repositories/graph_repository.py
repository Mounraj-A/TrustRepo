...
class GraphRepository:
    ...
    def setup_indexes(self):
        ...
        try:
            self.conn.query(
                "CREATE INDEX IF NOT EXISTS FOR (n:Class) ON (n.qualname)")
            self.conn.query(
                "CREATE INDEX IF NOT EXISTS FOR (n:Method) ON (n.qualname)")
            self.conn.query(
                "CREATE INDEX IF NOT EXISTS FOR (n:File) ON (n.qualname)")
        except Exception:
            pass
    ...
    def create_relationship_by_qualname(
            self, from_qualname: str, to_qualname: str, rel_type: str, properties: dict = None):
        if properties is None:
            properties = {}
        prop_string = ", ".join([f"{k}: ${k}" for k in properties.keys()])
        prop_clause = f" {{{prop_string}}}" if properties else ""
        ...
        """
        params = {
            "from_qualname": from_qualname,
            "to_qualname": to_qualname,
            **properties}
        return self.conn.query(query, params)
