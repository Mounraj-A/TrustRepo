...
class DecisionMatrix(BaseAgent):
    ...
    def process(self, message: AgentMessage) -> AgentMessage:
        ...
        # currently relies on known static pairs. Let's let it run its own or
        # we can inject.

        result = self.engine.evaluate(
            expected, observed, message.payload.get(
                "claim_text", ""))
        ...
        avg_quality = sum(e.get("confidence", 0.0)
                          for e in fused) / max(len(fused), 1)
        message.payload["evidence_quality"] = avg_quality
        message.payload["graph_connectivity"] = message.payload.get(
            "coverage_score", 0.0)
        message.payload["evidence_agreement"] = message.payload.get(
            "consistency_score", 0.0)

        self._log(
            message, f"Decision Matrix verdict: {
