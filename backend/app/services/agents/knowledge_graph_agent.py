...
class KnowledgeGraphAgent(BaseAgent):
    ...
    def process(self, message: AgentMessage) -> AgentMessage:
        ...
        self._log(
            message,
            f"Running reasoning graph traversal for features: {features}")

        evidence_chains = []

        # 1. Fetch FeatureInstances matching the semantic features
        # (We no longer fabricate evidence here; we just note the capability links if any)
        for feat in features:
            query = """
            MATCH (fi:FeatureInstance)-[:IMPLEMENTS]->(cap:Capability)
            WHERE fi.definition_id = $feat
            RETURN fi.id as id, fi.canonical_name as name, cap.name as capability
            """
            ...
            if results:
                for r in results:
                    chain = EvidenceChain(
                        ...
                        sequence=[] # No fabricated evidence snippets
                    )
        ...
        if len(features) > 1:
            for i in range(len(features) - 1):
                path = self.analytics.shortest_path(
                    features[i], features[i + 1])
                if path.reachable:
                    chain = EvidenceChain(
                        chain_id=f"kg_path_{features[i]}_{features[i + 1]}",
                        chain_type="Graph Analytics",
                        reasoning_trace=f"Shortest path found between {
                            features[i]} and {
                            features[
                                i +
                                1]} (Length: {
        ...
        message.confidence = min(
            0.5 + (len(evidence_chains) * 0.1) + bonus, 1.0) if evidence_chains else 0.1

        self._log(
            message, f"Found {
                len(evidence_chains)} graph evidence chains. Critical nodes found: {critical_nodes}")
