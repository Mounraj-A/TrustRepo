...
class ReasoningAgent(BaseAgent):
    ...
    def process(self, message: AgentMessage) -> AgentMessage:
        ...
        self._log(
            message,
            f"Expected {
                len(expected)} features, Observed {
                len(observed)} features. Ratio: {
                ratio:.2f}")
        if penalty > 0:
            self._log(message,
                      f"Applied contradiction penalty: -{penalty:.2f}")
        self._log(
            message,
            f"Final Reasoning Confidence: {
                reasoning_confidence:.2f}")
