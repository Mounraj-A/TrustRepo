...
from app.services.knowledge.technology_kb import resolve_technology, get_capabilities_for_tech, TECHNOLOGY_KB
    ...
    def detect(self, graph: RepositoryKnowledgeGraph) -> dict:
        ...
        for provider in self.providers:
            ...
            for item in items:
                ...
                matched_technologies = resolve_technology(
                    item.symbol, [file_lang] if file_lang != "Unknown" else [])

                # For annotations, we also need to check the hardcoded map for now if it's not in the KB's aliases directly
                # To maintain compatibility, we can add a fallback here, but
                # ideally they are in TECHNOLOGY_KB aliases
        ...
        for tech_id, items in tech_evidence_map.items():
            ...
            chain = EvidenceChain(
                ...
                sequence=items,
                graph_path=" -> ".join(
                    [item.evidence_type.value for item in items[:3]]),
                ranking_score=confidence,
                confidence=confidence,
                reasoning_trace=f"Found {
                    len(items)} pieces of evidence supporting {
                    tech.display_name}."
            )
