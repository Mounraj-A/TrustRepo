...
class ReasoningAgent:
    ...
    def evaluate(
        ...
        trace.append(f"[Stage 1 — Ontology] Normalized claim to expected features: {expected_features}")

        # ── Stage 2: Evidence Quality Assessment ──────────────────────────────
        quality_results = self.quality_assessor.assess_all(evidence_chains)
        aggregate_quality = self.quality_assessor.aggregate_quality(quality_results)
        for qr in quality_results:
            trace.append(
                f"[Stage 2 — Quality] Chain '{qr.chain_id[:8]}': "
                f"{qr.quality_label} (score={qr.quality_score:.2f}, class={qr.classification.value})"
            )

        # ── Stage 3: Evidence Validation (false positive + completeness) ──────
        tech_context = claim.text
        validation_results = self.validation_engine.validate_all(evidence_chains, tech_context)
        valid_chain_ids = {vr.chain_id for vr in validation_results if vr.is_valid}
        validated_chains = [c for c in evidence_chains if c.chain_id in valid_chain_ids]

        false_positives = [vr for vr in validation_results if vr.is_false_positive]
        trace.append(
            f"[Stage 3 — Validation] {len(validated_chains)} of {len(evidence_chains)} chains passed. "
        ...
        observed_features = self._extract_observed_features(validated_chains, expected_features)
        trace.append(f"[Stage 4 — Observation] Observed features: {observed_features}")

        # ── Stage 5: Decision Matrix ───────────────────────────────────────────
        dm_result = self.decision_matrix.evaluate(expected_features, observed_features, tech_context)
        trace.append(f"[Stage 5 — Decision Matrix]\n{dm_result.reasoning_trace}")
        ...
        for chain in validated_chains:
            for item in chain.sequence:
                ...
                if "import" in kind: evidence_types_found.add("Import detected")
                if "method" in kind or "function" in kind: evidence_types_found.add("Function calls detected")
                if "class" in kind: evidence_types_found.add("Class usage detected")
                if "dependency" in kind: evidence_types_found.add("Dependency found")
                if "annotation" in kind or "decorator" in kind: evidence_types_found.add("Annotation/Decorator verified")
        ...
        base_confidence = (0.4 * aggregate_quality) + (0.3 * dm_result.consistency_score)
    ...
    def _map_verdict(self, decision_verdict: DecisionVerdictState) -> VerificationVerdict:
    ...
    def _infer_category(self, claim_text: str) -> VerificationCategory:
        ...
        if any(k in claim_lower for k in ["security", "auth", "jwt", "oauth", "login"]):
            return VerificationCategory.SECURITY
        if any(k in claim_lower for k in ["database", "jpa", "sql", "mongo", "redis"]):
            return VerificationCategory.DEPENDENCY
        if any(k in claim_lower for k in ["microservice", "monolith", "architecture", "layer"]):
            return VerificationCategory.ARCHITECTURE
        if any(k in claim_lower for k in ["api", "endpoint", "rest", "graphql"]):
